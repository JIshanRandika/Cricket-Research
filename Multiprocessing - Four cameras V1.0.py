...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(processes=4) as pool:
        ...
        results = pool.map(run_function, [1, 2, 3, 4])
    ...
    # Combine results from all four cameras
    # ======================= c 
    # if dataframes:
    #     combined_dfC = pd.concat(dataframes, axis=1)
    # try:
    #     # Create figure with specified size
    #     plt.figure(figsize=(12, 6))
            
    #     # Get column names
    #     x1_name = combined_dfC.columns[0]
    #     y1_name = combined_dfC.columns[1]
    #     x2_name = combined_dfC.columns[2]
    #     y2_name = combined_dfC.columns[3]
            
    #     # Convert to numeric, with error handling
    #     combined_dfC = combined_dfC.apply(pd.to_numeric, errors='coerce')
            
    #     # Create the plots
    #     plt.plot(combined_dfC[x1_name], combined_dfC[y1_name], label='LKnee_Camera1')
    #     plt.plot(combined_dfC[x2_name], combined_dfC[y2_name], label='LKnee_Camera2')
    ...
    #     chart_path = os.path.join(os.path.dirname(__file__), 'knee_angles_chart_combined_df.png')
    #     plt.savefig(chart_path)
    #     plt.show()
    #     print(f"Chart saved successfully at {chart_path}.")

    # except Exception as e:
    #     print(f"Error generating or saving the chart for combined_df: {str(e)}")
    #     # Print the data types of columns for debugging
    #     print("\nColumn data types:")
    #     print(combined_dfC.dtypes)
