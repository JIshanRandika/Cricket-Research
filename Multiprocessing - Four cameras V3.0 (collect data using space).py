...
def save_combined_data(dataframes, prev_global_value, lock):
    ...
    # Combine results from all four cameras - Concat
        ...
        try:
            # Save to Excel
            combined_dfT.to_excel(os.path.join(os.path.dirname(__file__), f'combined_knee_angles_Multiprocessing_BasedonTime_{prev_global_value}.xlsx'), index=False)
            print("Combined Excel file saved successfully.")
        except Exception as e:
            print("Error saving combined Excel file:", e)
        ...
        try:
            ...
            chart_path = os.path.join(os.path.dirname(__file__), f'combined_knee_angles_Multiprocessing_BasedonTime_chart_{prev_global_value}.png')
