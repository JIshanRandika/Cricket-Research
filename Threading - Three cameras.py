...
def check_cameras():
    ...
    if not cap1.isOpened() or not cap2.isOpened() or not cap3.isOpened():
        ...
        if cap3.isOpened():
            cap3.release()
        raise RuntimeError("Could not open all three cameras")
...
class PoseDetector:
    ...
    def process_camera(self):
        ...
        df.to_excel(os.path.join(os.path.dirname(__file__), 
                                f'{self.output_prefix}_angles.xlsx'), 
                   index=False)
...
def main():
    try:
        ...
        combined_df = pd.merge_asof(
            pd.merge_asof(df1, df2, on='Time', suffixes=('_cam1', '_cam2'), direction='nearest'),
            df3, on='Time', suffixes=('', '_cam3'), direction='nearest'
        )
        
        # Save combined results
        combined_df.to_excel(os.path.join(os.path.dirname(__file__), 
                                          'combined_angles.xlsx'),
                            index=False)
